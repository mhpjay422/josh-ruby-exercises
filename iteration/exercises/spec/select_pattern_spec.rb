RSpec.describe 'select pattern' do
  ...
  it 'test 5' do
    ...
    words_with_c = []
    furniture.each {|piece| words_with_c << piece if piece.include?("c")}
  ...
  it 'test 6' do
    ...
    words_with_c = {}
    furniture.each {|piece, name| words_with_c[piece] = name if name.include?("c")}
  ...
  it 'test 8' do
    ...
    two_words = {}
    meal.each {|type, name| two_words[type] = name if name.split(" ").length == 2}
  ...
  it 'test 10' do
    ...
    floats = {}
    items.each {|type, num| floats[type] = num if num.class == Float}
