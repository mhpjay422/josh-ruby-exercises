RSpec.describe 'inject pattern test' do
  ...
  it 'test 6' do
    ...
    toppings = []
    topping_calories.each {|topping, cal| toppings << topping.to_s}
  ...
  it 'test 7' do
    ...
    elements.each {|ele| sum_of_second_values += ele[1]}
  ...
  it 'test 9' do
    ...
    grades.each {|type, arr| final_grade += arr.sum / arr.size.to_f}
  ...
  it 'test 10' do
    ...
    menu.each do |item| 
      gluten_free_or_not = item[1][:gluten_free]
      item_ingredients = item[1][:flavors]
      gluten_free_or_not ? gluten_free_or_not = "(gluten free)" : gluten_free_or_not = "(non gluten free)"

      menu_mapped = item_ingredients.map.with_index do |ele, idx|
        idx == item_ingredients.length - 1 ? "and #{ele} #{item[0]}" : ele
      end
      ingredients = menu_mapped.join(", ")
      printable_menu += """- #{ingredients} #{gluten_free_or_not}\n"\
    end
