RSpec.describe 'group by pattern' do
  ...
  it 'groups by first letter' do
    ...
    words.each {|word| words_by_first_letter[word[0]] << word}
  ...
  it 'group by uniqueness' do
    ...
    words.each {|word| grouped[word.downcase] << word}
  ...
  it 'group by number of zeroes' do
    ...
    grouped = Hash.new {|h, k| h[k] = []}
    numbers.each do |num|
      countnum = num.to_s.split("").count {|ele| ele == "0"}
      grouped[countnum] << num 
    end
  ...
  it 'group by order of magnitude' do
    ...
    grouped = Hash.new {|h, k| h[k] = []}
    numbers.each do |num|
      numlength = num.to_s.length
      grouped[numlength] << num 
    end
