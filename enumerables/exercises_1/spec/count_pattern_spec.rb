RSpec.describe 'Count Pattern Test' do
  ...
  it 'words that are uppercase' do
    ...
    tally = 0
    words.each {|word| tally += 1 if word.upcase == word}
    expect(tally).to eq(3)
  end
  ...
  it 'even numbers' do
    ...
    tally = 0
    numbers.each {|number| tally += 1 if number % 2 == 0}
  ...
  it 'multiples of 5' do
    ...
    tally = 0
    numbers.each {|number| tally += 1 if number % 5 == 0}
    expect(tally).to eq(3)
  end
  ...
  it 'round prices' do
    ...
    tally = 0
    prices.each {|price| tally += 1 if price.to_i == price}
    expect(tally).to eq(2)
  end
